...
type Props =
  | {
      type: "Create";
      payloadInputs: SkillsCreate;
    }
  | {
      type: "Update";
      payloadInputs: SkillsUpdate;
      skillsId: string;
    };
...
const actionPage = async () => {
  ...
  if (props.type === "Create") {
    const i: SkillsCreate = inputs.value as SkillsCreate;
    i.createdAt = new Date().toISOString();
    i.updatedAt = new Date().toISOString();
    response = await skillsStore.createDocument(i);
  }

  if (props.type === "Update") {
    inputs.value.updatedAt = new Date().toISOString();
    response = await skillsStore.updateDocument(
      inputs.value,
      props.skillsId
    );
  }
...
<template>
  <div class="d-flex align-items-center mb-3">
    <div>
      ...
      <h1 class="page-header mb-0">Skills {{ type }}</h1>
    </div>
  </div>
