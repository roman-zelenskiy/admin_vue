...
<template>
	<!-- BEGIN pos -->
	<div class="pos pos-vertical pos-with-header pos-with-sidebar" v-bind:class="{ 'pos-mobile-sidebar-toggled': mobileSidebarToggled }">
		<!-- BEGIN pos-container -->
							...
							<template v-if="selectedTable && selectedTable.orders">
								<div class="pos-order py-3" v-for="order in selectedTable.orders">
									<div class="pos-order-product">
										<div class="img w-50px h-50px rounded" v-bind:style="{ backgroundImage: 'url('+ order.image +')' }"></div>
										<div class="flex-1">
											<div class="row">
												<div class="col-7">
													<div class="fs-6 fw-semibold">{{ order.title }}</div>
													<div class="fs-13px">${{ order.price }}</div>
													<div class="fs-13px text-body text-opacity-50"><div v-for="option in order.options"> - {{ option }}</div></div>
												</div>
												<div class="col-2">x{{ order.quantity }}</div>
												<div class="col-3 text-body fw-semibold text-end">${{ (parseFloat(order.price) * order.quantity).toFixed(2) }}</div>
											</div>
										</div>
									</div>
								</div>
